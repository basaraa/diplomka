...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["semestre"])&&isset($_POST["teacherId"])){
    $semestre =$_POST["semestre"];
    $teacherId= $_POST["teacherId"];
    $days=["pondelok","utorok","streda","štvrtok","piatok"];
    $selectedTeacher=selectTeacherById($conn,$teacherId);
    if ($selectedTeacher)
        echo '<h2 class="purple">Rozvrh pre '.mysqli_fetch_assoc($selectedTeacher)["name"].' v '.$semestre.'</h2>';
    echo '<table class="tabulka"><thead>
        ...
        <tbody>';
    foreach ($days as $day){
        $selected = selectSubjectsByTeacherDay($conn,$teacherId,$semestre,$day);
        if ($selected) {
            $hour=5;
            echo '<tr>
